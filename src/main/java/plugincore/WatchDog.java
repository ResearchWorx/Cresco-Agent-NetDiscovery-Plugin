...
public class WatchDog {
	...
	public WatchDog() {
		  ...
		  timer = new Timer();
	      timer.scheduleAtFixedRate(new WatchDogTask(), 500, Integer.parseInt(PluginEngine.config.getParam("watchdogtimer")));
	...
	class WatchDogTask extends TimerTask {
	    public void run() {
	    	...
	    	long runTime = System.currentTimeMillis() - startTS;
	    	 ...
	    	 MsgEvent le = new MsgEvent(MsgEventType.WATCHDOG,PluginEngine.region,null,null,"WatchDog timer set to " + Integer.parseInt(PluginEngine.config.getParam("watchdogtimer")) + " milliseconds");
	    	 le.setParam("src_region", PluginEngine.region);
			 le.setParam("src_agent", PluginEngine.agent);
			 le.setParam("src_plugin", PluginEngine.plugin);
			 le.setParam("dst_region", PluginEngine.region);
